...
Console.prototype.callback = function(modulename, level, message, obj) {
  ...
  var milliseconds = (new Date).getTime() - startDate.getTime();

  var messageColor = this.colorFunctions[level];
  var prefix = this.prefixes[level];
  console.log(timeDiff(startDate, new Date)+' '+messageColor(prefix)+' '+this.moduleColor(modulename)+' '+messageColor(message));
};
